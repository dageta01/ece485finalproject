...
//  LPC_ADC->ADINTEN     =  (1<<8);        /* global enable interrupt       */
  LPC_ADC->ADINTEN     =  (1<<2);        /* CHANNEL 2 enable interrupt       */
...
//  AD_last = (LPC_ADC->ADGDR >> 4) & ADC_VALUE_MAX; /* Store converted value   */
	 ...
	 while (!(LPC_ADC->ADDR2 & ( 1UL << 31)));  /* Wait for Conversion end       */
  AD_last = (LPC_ADC->ADDR2 >> 4) & ADC_VALUE_MAX; /* Store converted value   */
...
void ADC_IRQHandler(void) {
  volatile uint32_t adstat,adlast2;

  //adstat = LPC_ADC->ADSTAT;		             // EXTRA STATEMENT

//  AD_last = (LPC_ADC->ADGDR >> 4) & ADC_VALUE_MAX; /* Store converted value   */
  AD_last = (LPC_ADC->ADDR2>>4) & ADC_VALUE_MAX;
  AD_done = 1;
}
#endif
